			...
			<table border>
				...
				<c:forEach var="vo" items="${list }">
					<tr>
						...
						<td>${vo.psDate }</td>
						<td><a
							href="freeView.jsp?psNo=${vo.psNo }&cPage=${pvo.nowPage}">
								${vo.psTitle } </a></td>
						<td>
							<form action="postDelete.jsp" method="get">
								<input class="h_button" type="button" value="삭제"
									onclick="ps_delete(this.form)"> <input type="hidden"
									name="psNo" value="${vo.psNo }"> <input type="hidden"
									name="no" value="${vo.no }">
							</form>
						</td>
					...
					<tr>
						<td colspan="4">
							<ol class="paging">
								...
								<c:if test="${pvo.beginPage != 1 }">
									<li><a href="free.jsp?cPage=${pvo.endPage - 1 }">이전</a></li>
								</c:if>
								...
								<c:if test="${pvo.endPage < pvo.totalPage }">
									<li><a href="free.jsp?cPage=${pvo.endPage + 1 }">다음</a></li>
								</c:if>
								<c:if test="${pvo.endPage >= pvo.totalPage }">
